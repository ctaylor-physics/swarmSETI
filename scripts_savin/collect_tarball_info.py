...
def main(filename):
    ...
    inputTGZB = os.path.basename(inputTGZ)
    meta = {inputTGZB : {}}
    ...
    meta[inputTGZB]["site"] = site.name
    meta[inputTGZB]["lst"] = lst
    ...
    meta[inputTGZB]["sess_dur"] = "%s" % sessionDur
    ...
    meta[inputTGZB]["obs_start"] = min(tStart).strftime(_FORMAT_STRING)
    meta[inputTGZB]["obs_end"] = (max(tStart) + lastDur).strftime(_FORMAT_STRING)
    ...
    meta[inputTGZB]["datafile"] = fileInfo[obsID]['tag']
    ...
    for i in range(nObs):
        ...
        print(" Ra, dec, %s, %s" % (project.sessions[0].observations[i].ra, project.sessions[0].observations[i].dec))
        ...
        meta[inputTGZB]["obs-%d" % i ] = {"target":project.sessions[0].observations[i].target, "ra": project.sessions[0].observations[i].ra, "dec": project.sessions[0].observations[i].dec, "mjd": project.sessions[0].observations[i].mjd, "start_time": get_observation_start_stop(project.sessions[0].observations[i])[0].strftime(_FORMAT_STRING), "duration": "%s" % currDur, "tun1_freq": project.sessions[0].observations[i].frequency1/1e6, "tun2_freq": project.sessions[0].observations[i].frequency2/1e6, "mode": project.sessions[0].observations[i].mode}
...
if __name__ == "__main__":
    ...
    path = "/leo/savin/SETI/data/alltar/DD002_*"
    tarfiles = sorted(glob.glob(path))
    with open("metadata.csv", "w") as fh:
        fh.write(",".join(
            ['tarball','beamid', 'datafile', 'site', 'obs_start', 'mjd', 'tun1_freq', 'tun2_freq','target',
            'duration', 'start_time','ra', 'dec', 'mode', 'lst', 'obs_end', 'sess_dur'
            ]
        )+"\n")
        ...
        for tarfile in tarfiles:
            ...
            dict = main(tarfile)
            print(dict)
            tar = list(dict.keys())[0]
            fh.write(f"{tar}, {dict[tar]['beamid']}, {dict[tar]['datafile']}, {dict[tar]['site']}, {dict[tar]['obs_start']}, {dict[tar]['obs-0']['mjd']}, {dict[tar]['obs-0']['tun1_freq']}, {dict[tar]['obs-0']['tun2_freq']}, {dict[tar]['obs-0']['target']}, {dict[tar]['obs-0']['duration']}, {dict[tar]['obs-0']['start_time']}, {dict[tar]['obs-0']['ra']}, {dict[tar]['obs-0']['dec']}, {dict[tar]['obs-0']['mode']}, {dict[tar]['lst']}, {dict[tar]['obs_end']}, {dict[tar]['sess_dur']} \n ")

            #fh.write(f"{tar}, {dict[tar]['beamid']}, {dict[tar]['datafile']}")
