...
h5_list = sorted(glob.glob(os.path.join(data_dir, '*.fil')))
...
for h5file in h5_list:
    ...
    fb = Waterfall(h5file)
    print(fb.header)
    fb.header['source_name'] = 'random'
    print(fb.header)
    fb.write_to_fil("/data/local/savin/seti/test/test1/corrected_1_new.fil")
...
"""
    ...
    if nint > 4: # Only runnin the doppler search if the number of time integration > 4
       ...
       doppler = FindDoppler(h5file,
                      ...
                      #gpu_backend = False,
                      #gpu_id = 0,
                      snr = 6,      # Minimum signal to noise ratio = 10:1
                      n_coarse_chan = 16 , # Blimpy calculate this for earlier BL data, make sure each file has correct coarse channels
                     )
       doppler.search()
t2 = time.time()
